...
const Movies = () => {
  ...
  const handleUploadSearchQuery = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert('Failed to upload search query: ' + (error.error?.message || 'Unknown error'));
      }
    } catch (err) {
  ...
  const handleAddRating = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const error = await res.json().catch(() => ({}));
        alert('Failed to add rating: ' + (error.error?.message || 'Unknown error'));
      }
    } catch (err) {
  ...
  const handleUnrateMovie = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const error = await res.json().catch(() => ({}));
        alert('Failed to remove rating: ' + (error.error?.message || 'Unknown error'));
      }
    } catch (err) {
  ...
  return (
    <div className="section fade-in">
      ...
      <div style={{ position: 'fixed', bottom: '2rem', right: '2rem', display: 'flex', flexDirection: 'column', gap: '0.8rem', zIndex: 1000 }}>
        <button
          ...
          >
            ...
            <form onSubmit={handleUpload}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px', marginBottom: '28px' }}>
                <div>
                  <label style={{ display: 'block', marginBottom: '8px', fontWeight: 600, fontSize: '15px', color: '#000' }}>
                    Movie Title *
                  </label>
                  <input
                    type="text"
                    ...
                    style={{ width: '90%', padding: '14px', border: '1.5px solid #000', borderRadius: '10px', fontSize: '15px', color: '#000' }}
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    style={{ width: '90%', padding: '14px', border: '1.5px solid #000', borderRadius: '10px', fontSize: '15px', color: '#000' }}
                  />
                  <small style={{ color: '#000', fontSize: '12.5px', marginTop: '6px', display: 'block' }}>
              ...
              <div style={{ marginBottom: '28px' }}>
                ...
                <textarea
                  ...
                  style={{ width: '100%', padding: '14px', border: '1.5px solid #000', borderRadius: '10px', fontSize: '15px', resize: 'vertical', color: '#000' }}
                />
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px', marginBottom: '32px' }}>
                <div>
                  ...
                  <div style={{ position: 'relative', border: '2px dashed #000', borderRadius: '10px', padding: '16px', textAlign: 'center', background: '#fafafa' }}>
                    <input
                      type="file"
                      name="coverImage"
                      accept="image/*"
                      ...
                      required
                      style={{ position: 'absolute', inset: 0, opacity: 0, cursor: 'pointer', color: '#000' }}
                    />
                    <div style={{ pointerEvents: 'none' }}>
                      {formData.coverImage ? formData.coverImage.name : 'Choose poster image...'}
                    </div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div style={{ position: 'relative', border: '2px dashed #000', borderRadius: '10px', padding: '16px', textAlign: 'center', background: '#fafafa' }}>
                    ...
                    />
                    <div style={{ pointerEvents: 'none' }}>
                      {formData.video ? formData.video.name : 'Choose movie file...'}
                    </div>
                  </div>
                </div>
              </div>
              ...
              <button
                ...
                style={{
                  ...
                  fontSize: '17px',
                  fontWeight: 600,
                  background: uploading ? '#aaa' : 'linear-gradient(90deg, #667eea 0%, #764ba2 100%)',
                  color: 'white',
                  border: 'none',
                  borderRadius: '10px',
                  cursor: uploading ? 'not-allowed' : 'pointer',
                  boxShadow: uploading ? 'none' : '0 4px 14px rgba(102,126,234,0.35)',
                }}
              >
        ...
        <div className="modal-overlay" onClick={() => setShowSearchModal(false)}>
          <div className="upload-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleUploadSearchQuery}>
              ...
              <button type="submit" className="btn upload-btn">Upload Query</button>
            </form>
          </div>
        </div>
      )}
          ...
          <div className="upload-modal" onClick={(e) => e.stopPropagation()}>
              ...
              <form onSubmit={handleAddRating}>
                <div className="form-group">
                  ...
                  <input
                    ...
                    onChange={(e) => setRatingForm({ ...ratingForm, unique_id: e.target.value })}
                    required
                  />
                </div>
                <div className="form-group">
                ...
                <button type="submit" className="btn upload-btn">Add Rating</button>
              </form>
            </div>
            ...
            <div>
              ...
              <form onSubmit={handleUnrateMovie}>
                ...
                <button type="submit" className="btn delete-btn">Remove Rating</button>
              </form>
            </div>
          </div>
        </div>
